function loadRecorderJS(callback) {
...
function isNull(test, value) {
  if (test != undefined && test != null) value = test;
...
function showMessage(type, message, config) {
  ...
  if (config.timeout != null) clearTimeout(config.timeout);
...
class AudioRecorderCard extends HTMLElement {
  set hass(hass) {
    if (!this.content) {
      ...
      if (header.length > 0) card.header = header;
...
#container {
  ...
  setConfig(config) {
    if (!config.target) throw new Error('Please define a target url for your record');
